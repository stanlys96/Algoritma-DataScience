...
_The Augmented Dickey-Fuller (ADF)_ test is a statistical test commonly used to test for the presence of a unit root in a time series. A unit root is a characteristic of a time series process where the mean and variance of the series increase over time and the series does not converge to a constant value, making it non-stationary.
...
Another way to determine more objectively if differencing is required is to use a
the _Kwiatkowski Phillips Schmidt and Shin test (KPSS) test_ test. That based on linear regression which breaks the time series into deterministic trend, random walk, and stationary error.

KPSS test is contrary to ADF test to test stationarity the null-hypothesis for an KPSS test is that the data are stationary, so **large p-values are indicative of stationarity** and small p-values suggest no-stationarity. Using the usual 95% threshold, differencing is required if the p-value is less than 0.05. 

```{r}
kpss.test(elec_ts)
kpss.test(souvenir_ts)
```
...
```{r echo=FALSE}
knitr::include_graphics("assets/adf&kpss.jpg")
```

There are many cases where both give contradicting results, so we follow a rule[^13]:

> Neither the ADF test nor the KPSS test will confirm or disconfirm stationarity in isolation. Run both tests to decide if you should difference. **If a least one of the tests claims to have found no-stationarity, you should difference**.

**Discuss:**
From the two time series, Are they stationary? How would you transform the time series into one that is stationary? Use the ADF & KPSS Test to confirm that your transformed time series is in fact now stationary:
```{r eval=F}
### Your answer below:
...
[^13]: [Tannyasharma, _Comparison Stydu of ADF vs KPSS test_](https://medium.com/@tannyasharma21/comparision-study-of-adf-vs-kpss-test-c9d8dec4f62a)
