...
🌐 [seq.Date() documentation](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/seq.Date)
...
Perhatikan bahwa terdapat pola yang berulang untuk setiap tahun. Pola tersebut disebut dengan **seasonality**. Maka, berdasarkan visualisasi diatas, jumlah penumpang pesawat **bulanan** memiliki seasonality dengan frequency = 12
...
## 🛵 Study Case: Motor Vehicle Retail Sales (DAUTONSA) 
...
- Bagaimanakah trend penjualan kendaraan bermotor?
    > Jawab : secara garis besar, ada kenaikan dari 2011 sampai 2015. tahun selanjutnya cenderung turun
    
- Apakah terdapat pola berulang yang terjadi? 
    > Jawab : disetiap awal tahun, ada pola penjualan yang  mengalami penurunan dan kenaikan cukup signifikan
...
- [X] ts(retail$sales, frequency = 365)
...
- [X] Mengetahui nilai di masa depan.
...
📝 Note: **Moving Average (MA)** akan dibahas pada materi **ARIMA**
...
💡 **Insight**: ada kenaikan yang disusul dengan penurunan. 
...
💡 **Insight**: Tiap bulan Maret selalu meningkat tajam salesnya. Paling rendah di bulan Januari.
...
📝 **Note**: Data AirPassengers merupakan data jumlah penumpang bulanan pesawat terbang pada sebuah maskapai di AS dari Januari 1949 hingga Desember 1960
...
1. **Simple Exponential Smoothing**
    ...
    - cocok untuk data yang **❌trend** & **❌seasonality**
...
💡 Insight: data `rain_ts` tidak memiliki trend dan tidak memiliki seasonal
...
💡 Insight: data `co2_ts` memiliki trend meningkat, dan tidak memilliki seasonality
...
Memutuskan apakah suatu model performanya bagus atau tidak bisa dilihat dari rata-rata dan standar deviasi data aktual. Secara bisnis, apakah error yang dihasilkan masih masuk akal dan tidak merugikan? 
```{r}
print(mean(sales_ts))
print(sd(sales_ts))
```
...
Berdasarkan model dengan nilai default dari Holt-Winters, yang memiliki performa terbaik adalah model Double Exponential (jika melihat MAPE). Simple Exponential (jika melihat MAE)
...
Berdasarkan visualisasi, model Holt-Winters dengan gamma = 0.6 lebih baik dibanding model Holt-Winters default
...
- [ ] Simple Exponential Smoothing
- [X] Holt Exponential Smoothing (Double Exponential Smoothing)
- [ ] Holt-Winters Exponential Smoothing (Triple Exponential Smoothing)
...
- [ ] Simple Exponential Smoothing
- [ ] Holt Exponential Smoothing (Double Exponential Smoothing)
- [X] Holt-Winters Exponential Smoothing (Triple Exponential Smoothing)

**Summary Workflow Time Series Forecasting**

1. Data preparation
  - Data sudah urut, memiliki interval yang tetap, dan tidak ada missing value
  
2. Ubah data TS menjadi objek TS dengan fungsi ts()
  - frekuensi harus disesuaikan dengan pola yang ingin diamati
3. Analisis dan EDA Time Series
  - Visualisasi
  - Decomposition
    + Trend Analysis
      - pergerakan naik, turun, atau datar
    + Seasonality
      - apakah ada pola yang berulang. dan apakah ada lebih dari satu seasonality
    + Error
4. Model Fitting
  + Simple Exponential (no trend, no season)
  + Double Exponential (yes trend, no season)
  + Triple Exponential (yes trend, yeas season)
  
5. Evaluasi model
  + Visualisasi
  + metrik MAE, RMSE, MAPE, dll
  + bisa dibandingkan dengan rata-rata dan standar deviasi dari data asli
  
6. Hyperparameter Tuning
  + mengubah parameter bobot (alpha, beta, gamma) di model exponential smoothing
...
- Interpretasi data lag untuk prediksi nilai target: Seperti ketika ingin memprediksi cuaca hari ini berdasarkan cuaca kemarin, jika kemarin hujan maka hari ini kemungkinan akan hujan juga

📝 **Notes**:

- `co2_emission` merupakan variabel target 
...
📝 **Note**: Nilai `NA` pada data pertama dan kedua disebabkan karena tidak adanya nilai 3 periode sebelumnya.
...
Ubah periode yang dirata-ratakan menjadi 10 tahun dan perhatikan perbedaannya:
...
📝 Insight: jika kita menggunakan data emisi co2 di Indonesia per tahunnya, akan lebih baik jika hanya menggunakan 3 data ke belakang saja, daripada 10 periode (10 tahun) ke belakang.
...
Dari hasil perhitungan di atas, apakah data kita sudah dianggap stasioner? belum stasioner.
...
Setelah 1x differencing, `co2_ts` belum stasioner

📝 Note: Jumlah differencing bisa lebih dari 1 kali, jika data belum stasioner setelah 1x differencing
...
Setelah 2x differencing, `co2_ts` sudah stasioner
...
Kita akan mencoba untuk membuat model `auto.arima()` untuk melakukan forecasting pada data `co2_ts` yang memiliki **trend** ✅ , namun tidak memiliki **seasonality** ❌
...
📝 **Note**: Data hasil differencing akan dikembalikan menjadi data time series aslinya ketika melakukan  forecasting 
...
📝 **Note** : Untuk percobaan lebih lanjut, silahkan gunakan nilai lag yang signifikan lainnya untuk melakukan hyperparameter tuning
...
Note: 
- 3 -> nilai plot PACF yang korelasinya signifikan dari lag 1 sampai lag 3 (ambil paling besar)
- 2 -> nilai berapa kali melakukan differencing
- 1 -> nilai plot ACF yang korelasinya signifikan hanya lag 1 saja
...
* p = berapa banyak lag yg digunakan dalam melakukan autoregresive -> Plot PACF
* d = berapa kali data di differencing hingga stationer -> ADF Testing
* q = berapa banyak lag yg digunakan untuk melakukan smoothing -> Plot ACF
...
💡 Berdasarkan visualisasi dan adf.test, data `sales_train` belum stationer


📝 **Notes**: Untuk differencing seasonal, kita akan gunakan parameter `lag` yang nilainya mengikuti frekuensi data yang kita miliki
...
```
p-value 0.3255 > alpha 0.05 

💡 Setelah dilakukan differencing seasonality, data `sales_train` belum stationer


📝 **Notes**: Jika setelah differencing seasonality data masih belum stasioner karena masih ada **trend**, lakukan differencing biasa untuk menghilangkan trend-nya
...
💡 Setelah dilakukan 1x differencing trend dan 1x differencing seasonality, data `sales_train` sudah stationer

Quick Summary:

- diff pertama memiliki lag=12 untuk melakukan differencing seasonality (karena seasonality tahunan)
- diff kedua tanpa parameter lag (lag=1) untuk melakukan differencing trend (karena ada trend turun)
...
- Note: lag 12 menjadi lag 1, lag 24 menjadi lag 2, lag 36 menjadi lag 3 dan seterusnya...
- Note: sama seperti parameter untuk trend, jika di awal sudah tidak signifikan, maka tidak perlu menggunakan lag (lag yg signifikan selanjutnya diabaikan saja)
...
💡 Model yang memiliki performa lebih baik pada `sales_test` adalah sama saja atau on par.
...
Kesimpulan: memenuhi asumsi no-autocorrelation residual.
...
Kesimpulan: residual tidak menyebar normal
