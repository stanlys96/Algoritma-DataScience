...
As a precautionary measure, I want to make sure that all the 353 "terms" in our train and test datasets are represented at least once. Verifying that both the train and test dataset have essentially the same number of columns (although this is not required for Naive Bayes to work). 
...
The following dataset is originally donated to the UCI Machine Learning Repository and organized by Friedrich Leisch. It contains 376 observations of variables such as `age`, `pregant`, `glucose` etc and a binary target variable (`pos` or `neg`):
```{r}
